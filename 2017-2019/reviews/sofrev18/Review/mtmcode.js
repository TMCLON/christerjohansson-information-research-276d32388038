...
var MTMCodeFrame = "code", MTMenuFrame = "menu", MTMTableWidth = "100%", MTMenuImageDirectory = "menu-images/", MTMenuBackgroundDirectory = "";
...
function MTMDisplayMenu() {
	if(MTMUA.MTMable && !MTMUpdating) {
		...
		if(MTMUA.DOMable) {
			...
			if(MTMBackground != "") {
				MTMOutputString += 'background="' + MTMUA.preHREF + MTMenuBackgroundDirectory + MTMBackground + '" ';
			}
...
function MTMcreateStyleSheet() {
	...
	if(MTMFirstRun && MTMUA.DOMable) {
		with(MTMUA.document.body) {
			...
			aLink = MTMLinkColor;
			if(MTMBackground) {
				background = MTMUA.preHREF + MTMenuBackgroundDirectory + MTMBackground;
			}
		}
